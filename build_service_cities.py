...
TEMPLATE = _swap(TEMPLATE,
    ...
    '<h2 style="font-size:clamp(2rem,4vw,2.8rem);margin-bottom:18px">{dir_h2}</h2>')
TEMPLATE = _swap(TEMPLATE,
    '<h2>What to expect when you drive in from {city}</h2>',
    '<h2>{visit_h2}</h2>')
TEMPLATE = _swap(TEMPLATE,
...
def build_page(p):
    ...
    return TEMPLATE.format(
        ...
        dir_h2=p.get("dir_h2") or ("From %s to Serene Med Spa" % p["city"]),
        visit_h2=p.get("visit_h2") or ("What to expect when you drive in from %s" % p["city"]),
        cta_h2=p.get("cta_h2") or ("Worth the drive from %s" % p["city"]),
