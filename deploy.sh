#!/usr/bin/env bash
# deploy.sh — git-based deploy for barboursville.serenemedspas.com
#
# Runs ON THE VPS from the cloned repo (default: /root/barboursville-src).
# It pulls the latest commit, rebuilds the site, promotes it atomically into
# the live directory (/root/barboursville/site), and restarts the nginx container.
# The live site is only touched AFTER a clean build + sanity check, so a bad
# push can never leave a half-written site.
#
# Usage on the VPS:
#     cd /root/barboursville-src && ./deploy.sh
#
# One-time setup lives in README.md ("Git-based deploy").
set -euo pipefail

# --- config (override with env vars if your paths differ) ---
SRC="${SRC:-/root/barboursville-src}"      # the git clone (this repo)
LIVE="${LIVE:-/root/barboursville}"        # where docker-compose.yml + live site/ live
BRANCH="${BRANCH:-main}"
...
docker ps --format 'table {{.Names}}\t{{.Status}}' | grep -E 'NAMES|barboursville' || true
