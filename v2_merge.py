# -*- coding: utf-8 -*-
"""v2_merge.py <site-dir> — FINAL build pass for the Barboursville site.

Phase 2 of folding the location sites into serenemedspas.com (Sep 2026):
  * every root-relative URL (/botox/, /img/x.jpg, /shop.js …) becomes /barboursville/…
  * https://barboursville.serenemedspas.com/… becomes https://serenemedspas.com/barboursville/…
    (canonicals, og:url, JSON-LD, sitemap.xml)
  * the Barboursville header/promo/footer/mobile bar are swapped for the main site's v2 shell
...
The built tree is then served by the MAIN nginx container at serenemedspas.com/barboursville/
(bind mount of /root/barboursville, `location /barboursville/ { alias … }`), and the
barboursville. subdomain only 301s (bundle/nginx.conf).
...
PREFIX = "/barboursville"
OLD_HOST = "https://barboursville.serenemedspas.com"
...
def build_v2_css():
    ...
    tokens = """
/* v2 tokens mapped onto the Barboursville stylesheet's variables */
...
def main():
    ...
    for path in glob.glob(os.path.join(SITE, "**", "*"), recursive=True):
        ...
        already = "/barboursville/" in s and OLD_HOST not in s and ext == ".html" and "v2-shell.js" in s   # merged on a previous run / template already carries the shell
        if ext == ".html":
            ...
            s = s.replace("https://hudson.serenemedspas.com/", NEW_HOST + "/").replace("https://hudson.serenemedspas.com", NEW_HOST)  # cloned-from-Hudson canonicals
        elif ext == ".css":
            s = prefix_css(s)
            for a, b in HOME_IMG_SWAP.items(): s = s.replace(a, b)
        elif ext == ".js":
